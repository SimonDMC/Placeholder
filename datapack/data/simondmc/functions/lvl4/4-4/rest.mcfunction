...
# banner craft give
execute as @a if score @s banner matches 1 run clear @s dead_fire_coral
execute as @a if score @s banner matches 1 run give @s white_banner{CanPlaceOn:["orange_concrete_powder"],display:{Name:'{"text":"White Banner","color":"#FFB217","italic":false}'}}
execute as @a if score @s banner matches 1 run scoreboard players set @s banner 2
...
execute as @a if predicate simondmc:lvl4/stone_stuck at @s unless block ~ ~-.1 ~ air unless block ~-1 ~1 ~ air unless block ~1 ~1 ~ air unless block ~ ~1 ~-1 air unless block ~ ~1 ~1 air run title @s actionbar {"text":">:(","color":"dark_red"}
execute as @a if predicate simondmc:lvl4/stone_stuck at @s unless block ~ ~-.1 ~ air unless block ~-1 ~1 ~ air unless block ~1 ~1 ~ air unless block ~ ~1 ~-1 air unless block ~ ~1 ~1 air run effect give @s levitation 1 3 true
execute as @a if predicate simondmc:lvl4/stone_stuck at @s if block ~-1 ~ ~ air if block ~1 ~ ~ air if block ~ ~ ~-1 air if block ~ ~ ~1 air run effect clear @s levitation

# level completion
execute as @a[nbt={Inventory:[{id:"minecraft:potion",tag:{Potion:"minecraft:long_leaping"}}]}] run scoreboard players set pot 4 1
...
execute unless score complete4 4 matches 1 if score pot 4 matches 1 run title @a actionbar "§eYou are missing some items you will need later."
...
execute as @a if score @s campfire_craft matches 1 run give @s campfire{CanPlaceOn:["coal_block"]}
execute as @a if score @s campfire_craft matches 1 run scoreboard players reset @s campfire_craft

# enter prototype 1 (narrator line)
execute unless score entered_p1 4 matches 1 if entity @a[x=-999,y=120,z=-994,dy=1] run scoreboard players set id narrator 12
execute unless score entered_p1 4 matches 1 if entity @a[x=-999,y=120,z=-994,dy=1] run scoreboard players set entered_p1 4 1

# enter prototype 2 (narrator line)
execute unless score entered_p2 4 matches 1 if entity @a[x=-1004,y=128,z=-978,dy=1] run scoreboard players set id narrator 13
execute unless score entered_p2 4 matches 1 if entity @a[x=-1004,y=128,z=-978,dy=1] run scoreboard players set entered_p2 4 1