...
execute as @a if score @s stone matches 1 if score pickaxe break matches 2 run clear @a wooden_pickaxe
execute as @a if score @s stone matches 1 if score pickaxe break matches 2 at @s run playsound entity.item.break master @s
...
execute as @a if score @s stone matches 1 run scoreboard players add pickaxe break 1
scoreboard players reset @a stone
scoreboard players reset @a coal

# stone dig down soft lock patch
execute as @a if predicate simondmc:lvl1/stone_stuck at @s unless block ~ ~-.1 ~ air unless block ~-1 ~1 ~ air unless block ~1 ~1 ~ air unless block ~ ~1 ~-1 air unless block ~ ~1 ~1 air run title @s actionbar {"text":">:(","color":"dark_red"}
execute as @a if predicate simondmc:lvl1/stone_stuck at @s unless block ~ ~-.1 ~ air unless block ~-1 ~1 ~ air unless block ~1 ~1 ~ air unless block ~ ~1 ~-1 air unless block ~ ~1 ~1 air run effect give @s levitation 1 3 true
execute as @a if predicate simondmc:lvl1/stone_stuck at @s if block ~-1 ~ ~ air if block ~1 ~ ~ air if block ~ ~ ~-1 air if block ~ ~ ~1 air run effect clear @s levitation