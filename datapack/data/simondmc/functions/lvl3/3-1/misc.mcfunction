...
execute if entity @a[tag=started,x=1019,y=112,z=1031,dy=1] run fill 1019 114 1032 1019 113 1033 air replace minecraft:barrier

# make campfire bottle on forever
data modify block 1008 101 1025 CookingTimes[0] set value -1000000

# dumb hole (narrator line)
execute unless score temp_dumb_hole narrator matches 1 as @a[tag=started,x=1026,y=101,z=1026,dy=1] run scoreboard players set id narrator 17
execute unless score temp_dumb_hole narrator matches 1 as @a[tag=started,x=1026,y=101,z=1026,dy=1] run scoreboard players set temp_dumb_hole narrator 1

# get out of dumb hole
execute if score dumb_hole_cheat narrator matches 1 if entity @a[tag=started,gamemode=!adventure] run function simondmc:lvl3/3-1/dumb_hole_cheat